...
class SpoolManagerAPI(octoprint.plugin.BlueprintPlugin):
	...
	def selectSpoolByQRCode(self, databaseId):
		self._logger.info("Parameters passed to selectSpoolByQRCode:")
		self._logger.info(request.args)

		requestParameters = request.args
		printerNumber = requestParameters["printerNumber"]

		self._logger.info("PrinterID:"+ str(printerNumber))
		...
		if("T" in str(printerNumber)): # e.g. #5T3
			...
			self._logger.info("stringprinternumber"+printerNumber)
			...
			self._logger.info("stringprinternumber after split"+printerNumber)
			self._logger.info("toolNumberZeroIndexed after split"+str(toolNumberZeroIndexed))
		...
		spoolModelAsDict = Transformer.transformSpoolModelToDict(spoolModel)
		json_object = json.dumps(spoolModelAsDict)
	...
	def generateSpoolQRCode(self, databaseId):
		...
		if (databaseId == "qrPreviewId" or self._databaseManager.loadSpool(databaseId) is not None):
			...
			textDraw.text((10, 15),displayNameBroken[0],fill="black",font=fontBig)
			displayNameBroken=displayNameBroken[1:]
			displayNameLower = "".join(displayNameBroken)
			textDraw.text((10,65), displayNameLower,fill="black",font=fontBig)
			#textDraw.text((10,120), material,fill="black",font=fontBig)
			textDraw.text((10,120), vendor,fill="black",font=fontSmall)
			textDraw.text((10,160), databaseId,fill="black",font=fontExtraBig)
			...
			label = Image.new(mode = "RGB", size = (475,355), color = backgroundColor)
